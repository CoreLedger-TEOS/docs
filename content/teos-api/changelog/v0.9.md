...
* Starting from this version TEOS API consumers can pay invoices in two ways:
  * Direct transfer of an invoice asset from own address to the invoice target address: `POST /odata/v0.9/Invoices({key})/PayWithTransfer`.
  * WARP execution against one of the owned assets in return for the invoice asset: `POST /odata/v0.9/Invoices({key})/PayWithWarpTransfer`.

TEOS API consumers should run WARP search first, find the WARP results that meet the invoice criteria, select the best one, and then submit WARP parameters to TEOS API to initiate `/PayWithWarpTransfer`:
...
`senderAddress` is the address paying for the invoice, `inputAmount` is the amount of offered asset taken from the WARP search result, and `supplyIds` lists the supplies to execute within the selected WARP.

Transaction parameters for further signing and submitting are returned as a response according to [transaction-creation-and-submission](../resources/dealing-with-blockchain-transactions/transaction-creation-and-submission/ "mention").
