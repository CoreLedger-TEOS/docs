...
👉 Now admin can adjust KYC settings and define whether tenant needs KYC cases to be automatically closed (approved/declined) based on the verification status received from KYC provider (note: valid only for integration with KYC provider, if "Manual" provider is used, all KYC cases must be closed manually). Read more in [KYC checks settings](../admin-user-guide/portal-configuration/features-configuration/kyc-checks.md#step-5-let-portal-close-kyc-cases-automatically-if-verification-is-made-by-third-party-kyc-service)

👉 Now admin can customize redemption functionality even more by defining the menu item name which is currently called "Redemption" and defining whether tenant needs additional details within redemptions. Read more in [Redemption general settings](../admin-user-guide/portal-configuration/features-configuration/redemption-functionality/index.md#step-1-configure-general-redemption-settings)
...
![](/img/gitbook-assets/wlp-version-016-redemption-values-step.png)

_New step values controls in case step value is defined by the admin_

👉 We've introduced French language in WLP! Now you can configure WLP to use it among available languages for your end users
