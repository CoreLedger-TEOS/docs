...
const additionalGroups = [
  {
    ...
    matches: [
      ...
      /^admin-user-guide\/(index|how-to-transfer-tokens-to-the-external-wallet|role-management|teos-platform-preparation|user-management)$/,
    ],
  },
  {
  ...
  },
  {
    ...
    matches: [
      ...
      /^admin-user-guide\/(login-and-wallet-creation|wla-modules)$/,
    ],
  },
];

const teosApiAdditionalMatches = [
  /^faq$/,
  /^glossary$/,
  /^get-started$/,
  /^overview\/dealing-with-blockchain-transactions(?:\/|$)/,
  /^overview\/(supply-management|warp-search)$/,
...
function buildTeosApiAdditionalItems(allDocIds, summaryDocIds) {
  const items = [...allDocIds]
    .filter((id) => !summaryDocIds.has(id) && !ignoredAdditionalDocIds.has(id))
    .filter((id) => matchesAnyPattern(id, teosApiAdditionalMatches))
...
function buildAdditionalItems(allDocIds, summaryDocIds, handledDocIds) {
  const extras = [...allDocIds]
    .filter((id) => !summaryDocIds.has(id) && id !== 'index' && !handledDocIds.has(id) && !ignoredAdditionalDocIds.has(id))
    .sort((a, b) => a.localeCompare(b));
  ...
  if (remaining.length) {
    grouped.push({
      type: 'category',
      label: 'Imported pages to review',
      collapsed: true,
...
function writeSidebar(allDocIds) {
  ...
  const teosApiItems = summary.map((item) => summaryItemToSidebar(item, allDocIds)).filter(Boolean);
  const teosApiAdditionalItems = buildTeosApiAdditionalItems(allDocIds, summaryDocIds);
  const handledDocIds = new Set([
    ...summaryDocIds,
    ...teosApiAdditionalItems.flatMap((category) => category.items.map((item) => item.id)),
  ]);
  const sidebarItems = [
    {
      type: 'category',
      label: 'TEOS API',
      collapsed: false,
      items: [...teosApiItems, ...teosApiAdditionalItems],
    },
  ];
  sidebarItems.push(...buildAdditionalItems(allDocIds, summaryDocIds, handledDocIds));
